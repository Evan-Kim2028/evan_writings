...
Contamination closes a pool the same way saturation does. OpenAI stopped reporting SWE-bench Verified on February 23, 2026, saying gains on it "increasingly reflect how much the model was exposed to the benchmark at training time" ([Why SWE-bench Verified no longer measures frontier coding capabilities](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/)). It named [SWE-bench Pro](https://github.com/scaleapi/SWE-bench_Pro-os) as the replacement. OpenAI retracted that on July 8, 2026 after flagging 200 of Pro's 731 tasks by pipeline and 249 by human review ([Separating signal from noise in coding evaluations](https://openai.com/index/separating-signal-from-noise-coding-evaluations/)). Both audits attempt this article's measurement, and both searched where they suspected trouble, so each yields a count rather than a rate.
