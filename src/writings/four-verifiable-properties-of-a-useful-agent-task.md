...
A low solve rate alone cannot tell a hard task from a broken one, so difficulty follows soundness rather than replacing it. Across the 1,699 SWE-bench tasks that 3 raters each annotated for OpenAI in 2024, the ones they called invalid sit at a median solve rate of 2% while sound tasks sit at 33%. Those labels are a floor rather than a verdict, since OpenAI later found its own 2024 screen had passed tasks whose tests reject correct solutions.
...
Nothing cheaper than measuring recovers the slope. Guessing it from a task's features fails. A model fitted on how tasks look and how agents behaved on them, asked which of two tasks from the same benchmark holds up better, gets it right 51% of the time, which is chance. Human review fails too. A durable task has a low slope, and the 80 Harbor-Index tasks the reviewers kept carry a median slope of 2.9 percentage points against 0.2 for the 1,262 they rejected. The kept tasks are if anything slightly steeper, and the gap is not statistically significant, so the reviewers were not selecting for durability at all.
...
Contamination retires a pool the same way saturation does. OpenAI stopped reporting SWE-bench Verified on February 23, 2026 ([Why SWE-bench Verified no longer measures frontier coding capabilities](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/)). Every frontier model it tested reproduced gold patches or problem statements verbatim, so gains on it "increasingly reflect how much the model was exposed to the benchmark at training time." That post named [SWE-bench Pro](https://github.com/scaleapi/SWE-bench_Pro-os), Scale's 731-task benchmark, as the successor. OpenAI retracted the recommendation on July 8, 2026 after flagging 200 of Pro's 731 tasks by pipeline and 249 by human review ([Separating signal from noise in coding evaluations](https://openai.com/index/separating-signal-from-noise-coding-evaluations/)).

Both audits try to quantify what this article quantifies, and neither produces a rate. The Verified pass covered the 138 tasks o3 failed across 64 runs and found 59.4% of them carried tests that reject correct solutions. Picking tasks by suspicion answers how many bad tasks a search can turn up, not how many the pool holds, which is the question a random draw and a bound answer. Neither release carries per-task IDs, so no one outside can join the verdicts, recheck them, or reuse them.
...
How many sound tasks that selection passed over depends on where the bar sits. Counting a task sound only when no rater objected leaves 539, so the selection drops 39 and amounts to a validity filter. Allowing one objection leaves 935, so it drops 435, and those 435 are the half that goes missing everywhere else. A model fitted to that looser split picks the kept task out of a pair 60% of the time on tasks it has not seen, against 50% for a coin flip. The signal is real but nowhere near strong enough to accept or reject a task on, and it rests on a threshold nobody has justified.
