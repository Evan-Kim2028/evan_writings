...
Contamination retires a pool the same way saturation does. OpenAI stopped reporting SWE-bench Verified on February 23, 2026, writing that gains on it "increasingly reflect how much the model was exposed to the benchmark at training time," after every frontier model it tested reproduced gold patches or problem statements verbatim ([Why SWE-bench Verified no longer measures frontier coding capabilities](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/)). That post named [SWE-bench Pro](https://github.com/scaleapi/SWE-bench_Pro-os), Scale's 731-task benchmark, as the successor. OpenAI retracted the recommendation on July 8, 2026 after flagging 200 of Pro's 731 tasks by pipeline and 249 by human review ([Separating signal from noise in coding evaluations](https://openai.com/index/separating-signal-from-noise-coding-evaluations/)).
