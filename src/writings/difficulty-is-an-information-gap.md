...
factory scales with compute instead of reviewers. This post describes a factory built on the ladder
and what its first 591 Go tasks show.
...
- **Two prompts separate a hard task from a broken one.** A task is certified only when one model
  fails the bug report and passes the full description. None of the comparable pipelines reports
  that second run.
...
- **The same ladder grades models.** Composer and Devin both fail `archive` from the bug report,
  and the ladder puts them two steps apart. Across 39 tasks both climbed, they need the same level
  on 23, and where they part ways the description tends to run long.
- **Money is the limit, not method.** Stage 1 graded 415 tasks with 7.9 billion tokens, worth
  $1.4k at API prices. Measuring every task at every level with both models would cost about $15k.
...
The prompt, by contrast, moves the outcome directly. Of the 415 graded tasks, 172 were solved from
the bug report. The other 242 failed it and passed higher up, and 197 of those passed as soon as
the full description arrived, with the code, the tests, and the model unchanged. Another 11 needed
the test names and 34 the test file. One task, `exprhash`, has beaten two models at every level.
...
run with the failed run just below it on the same task. The passing run takes fewer tool calls, 58 against 68 for Composer, and 57
against 68 for Devin, and nearly the whole saving is exploration. Composer's read and search calls
fall from 50 to 38 and Devin's from 43 to 30, a quarter to a third fewer, while test runs stay
level at 8 and 9. For Devin the exploring share of its calls drops too, from 64% to 56%. The
information does searching the model would otherwise have done.
...
Across all 1,646 graded runs the two models work much alike, a median of 62 tool calls, about two
thirds of them reading and searching and one in seven running the tests, though Devin takes 18
minutes a run to Composer's 3. Composer's failed runs take more calls than its passed runs, 73
against 54, so a long Composer run is an early sign of a miss that a budget could cut short, while
Devin's failed and passed runs look the same, 63 calls against 62. Failure changes shape as the
ladder rises. At the bug report and the
full description, Composer's failed runs are its long ones, 69 and 70 calls against 52 and 51 for
passes, with more edits, as it rewrites code that keeps failing. Higher up the order flips for both
models, and failures become the short runs. With the test file in the tree Composer fails in 44
calls and passes in 71, and Devin's failures are the shorter runs from the full description on.
...
model keeps climbing, and the certificate records the first level it passes. "Hard at L1" is a
statement about a model, so every certificate names its model. An earlier version took the lowest
passing level across all models, which let the stronger model erase the weaker one's difficulty and
put two certificates three levels too low.
...
Composer ran out of ladder. The budget allowed about one run per level, so each gap below is a
single observation.
...
solvable, and Devin passed `httpencoding` once every hidden test was in the tree. `exprhash` is
solvable by construction, since its answer key passes the suite, yet 28 runs from two models have
not solved it.
...
on the other 19 Devin passed where Composer failed. A second screen of certified tasks shows how
much the repository matters. Devin solved eight of ten `go-github` tasks from the bug report, whose
issues name the fields the code turns on, and nine of 65 everywhere else.
...
The bucket that failed every level held 29 tasks before the upper levels were measured. Every
audited one had a defective description, the rest certified once a higher level supplied what the
prose left out, and only `exprhash` remains.
...
record spending. The factory could author tasks and the harness could run them faster than those
allowances could pay for the runs, and the budget shaped the dataset more than any design choice
did.
...
Grading it was not, at 7.0 runs and $4.05 per certificate, and a run above L2 cost twice an L2 run,
$0.78 against $0.38, because only harder tasks reach those levels and the solver has a test file
to read. The factory made tasks faster than the budget could grade them, so 155 of the 591
...
Measuring everything, every authored task at every level three times by both models, is about 21k
runs and 83 billion tokens. That is $15k at these prices, eleven times Stage 1. The factory already
...
- **Selection shapes the model comparison.** Composer screened 374 of the 436 tasks that ran, and
  most tasks Devin saw were ones Composer had failed, so Devin's lean toward lower grades is partly which tasks it
  got.
- **The second model is thin.** A Devin run cost about twice a Composer run and drew on a fixed
  ...
  climbs. Grok ran only the three tasks at the top. The 172 tasks solved from the bug report carry
  one model's grade: Composer's on 132, Devin's on 36, and Grok's on 3.
- **Tasks are not independent.** The levels of a family nest by design, and tasks from one
  repository share its code and conventions. A training or evaluation split should keep each
  family, and where it can each repository, on one side.
- **The design limits memorization but cannot rule it out.** The answer keys are upstream code from widely used
  repositories. No task file references an issue, pull request, or CVE, so nothing in the prompt
  names the code to recall, and the answer keys are 103k added lines against 638 non-stub
  deletions, behavior to rebuild instead of a known diff to replay. A direct probe, asking a model
  for each function from its name alone, is the next check.
...
- **What does the curve above L2 look like?** 81% of certificates bind at the full description and
  ...
  76% of the time and Devin's 84%, and a second model climbing the same tasks would show whether
  that gap is the models or the selection.
